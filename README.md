...
- [`./scripts/launch-agent-pane.sh`](./scripts/launch-agent-pane.sh): opens Antigravity, Codex, fx, or Pi in a right-side pane rooted at the active pane directory.
...
The AI category also opens Antigravity, Codex, fx, and Pi as 45%-wide right-side panes in the active pane directory. Antigravity always starts a new session. Codex can start a new session or open its directory-filtered resume picker. fx can start a new session or resume its latest workspace session (`fx session resume last`). Pi can start a new session, continue its previous session (`pi --continue`), or open its interactive session picker (`pi --resume`). These actions require `agy`, `codex`, `fx`, or `pi` in `PATH`. They do not add dedicated tmux key bindings. Focus an agent pane and use `prefix + !` to move the running pane into its own window.
