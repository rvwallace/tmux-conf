...
pause_if_interactive() {
  ...
  if [ "${mode:-}" = "command" ] && [ "$status" -eq 0 ]; then
    return
  fi
...
case "$mode" in
  ask|error|command|explain|ask-buffer|command-buffer|explain-buffer) ;;
...
scrollback="$(tmux capture-pane -J -p -S -300 -t "$pane_id" 2>/dev/null || true)"
...
if [ "$mode" != "error" ] && [ -z "$prompt_text" ]; then
...
EOF
    ...
    tmux set-buffer -b tmux-ai-command "$command_text"
    tmux paste-buffer -t "$pane_id" -b tmux-ai-command
