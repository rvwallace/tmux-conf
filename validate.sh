...
expect_contains "tmux list-keys -T prefix | grep 'bind-key.*Space'" "scripts/tmux-menu.py" "leader key menu Space binding"
expect_contains "tmux list-keys -T root | grep 'bind-key.*C-p'" "scripts/tmux-menu.py" "command palette C-p root binding"
expect_contains "tmux list-keys -T prefix | grep 'bind-key.*P'" "scripts/tmux-menu.py" "command palette P prefix binding"
...
expect_contains "grep -F 'show-clients.sh' \"$REPO_DIR/tmux-menu/config.json\"" "show-clients.sh" "attached clients helper command"
