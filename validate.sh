...
expect_contains "grep -F 'aichat-tui.py' \"$REPO_DIR/scripts/ai-prompt.sh\"" "aichat-tui.py" "ai-prompt delegates to aichat-tui"
expect_contains "grep -F '#!/usr/bin/env -S uv run --script' \"$REPO_DIR/scripts/aichat-tui.py\"" "uv run --script" "AI TUI shebang metadata"
expect_contains "grep -F 'textual' \"$REPO_DIR/scripts/aichat-tui.py\"" "textual" "AI TUI Textual dependency"
expect_contains "grep -F 'TMUX_AI_ASSIST_FOLLOW_UP' \"$REPO_DIR/scripts/aichat-tui.py\"" "TMUX_AI_ASSIST_FOLLOW_UP" "AI follow-up context control"
expect_contains "grep -F 'TMUX_AI_ASSIST_REFRESH' \"$REPO_DIR/scripts/aichat-tui.py\"" "TMUX_AI_ASSIST_REFRESH" "AI conversation context refresh"
...
expect_contains "grep -F '\"title\": \"AI: Open Antigravity Agent\"' \"$REPO_DIR/tmux-palette/commands.json\"" "AI: Open Antigravity Agent" "Antigravity agent palette entry"
expect_contains "grep -F '\"title\": \"AI: Open Codex Agent\"' \"$REPO_DIR/tmux-palette/commands.json\"" "AI: Open Codex Agent" "Codex agent palette entry"
expect_contains "grep -F '\"title\": \"AI: Resume Codex Agent\"' \"$REPO_DIR/tmux-palette/commands.json\"" "AI: Resume Codex Agent" "Codex resume palette entry"
expect_contains "grep -F '\"description\": \"Agents\"' \"$REPO_DIR/tmux-which-key/config.json\"" "Agents" "AI agents which-key group"
expect_contains "grep -F 'launch-agent-pane.sh antigravity' \"$REPO_DIR/tmux-which-key/config.json\"" "launch-agent-pane.sh antigravity" "Antigravity which-key command"
expect_contains "grep -F 'launch-agent-pane.sh codex-resume' \"$REPO_DIR/tmux-which-key/config.json\"" "launch-agent-pane.sh codex-resume" "Codex resume which-key command"
